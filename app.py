...
st.caption("Priority-first resource scheduling. Allocation order = Critical > High > Normal > Low, then earliest requirement window when priority ties.")
...
with tab_jobs:
    ...
    st.subheader("Jobs")
    # Re-fetch so the table reflects any just-created job after rerun
...
with tab_requirements:
    ...
    # FIX: always re-fetch jobs here via get_jobs_df so job_end_date (computed
    # in Python, not stored in DB) is guaranteed to be present on the DataFrame.
    jobs_df = get_jobs_df(engine)
    if jobs_df.empty:
        ...
        with st.form("create_requirement_form", clear_on_submit=True):
            ...
            if submitted:
                try:
                    ...
                    st.success("Requirement added and priority-based allocation applied.")
                    st.rerun()
                except Exception as e:
    ...
    with st.form("upsert_pool_form"):
        ...
        if submitted:
            try:
                ...
                st.success(
                    f"Pool saved and allocations rebalanced: {region_code} / {selected_rc['class_name']} / {submitted_qty} {selected_rc['unit_type']}"
                )
                st.rerun()
            except Exception as e:
    ...
    with st.form("pool_adjustment_form", clear_on_submit=True):
        ...
        if submitted:
            try:
                ...
                st.success("Adjustment added and allocations rebalanced.")
                st.rerun()
            except Exception as e:
                st.error(f"Adjustment save failed: {e}")

    c1, c2 = st.columns([1, 1])
    if c1.button("Rebalance All Allocations"):
        try:
            rebalance_all(engine)
            st.success("All allocations rebalanced.")
            st.rerun()
        except Exception as e:
            st.error(f"Rebalance failed: {e}")

    as_of_date = c2.date_input("Pool Snapshot As Of", value=date.today(), key="snapshot_date")
...
with tab_allocations:
    ...
    debug_df = allocation_debug_df(engine)
    if debug_df.empty:
        st.info("No debug rows yet.")
    else:
        st.dataframe(format_dates_for_display(debug_df), use_container_width=True)
    ...
    if mode == "Availability":
        ...
        if req_summary.empty:
            ...
            fig = px.timeline(
                ...
                y="job_code",
                color="class_name",
                hover_data=["job_name", "region_code", "priority", "quantity_required", "unit_type", "allocation_status"],
            )
        ...
        if ful.empty:
            ...
            fig = px.timeline(
                ...
                hover_data=["job_name", "region_code", "priority", "class_name", "quantity_assigned", "unit_type", "source_name"],
            )
            fig.update_yaxes(autorange="reversed")
            st.plotly_chart(fig, use_container_width=True)
...
with tab_gantt:
    ...
    if req_summary.empty:
        ...
        fig = px.timeline(
            ...
            hover_data=["priority", "quantity_required", "quantity_assigned", "quantity_shortfall"],
        )
        fig.update_yaxes(autorange="reversed")
        st.plotly_chart(fig, use_container_width=True)

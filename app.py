...
with tab_jobs:
    ...
    if jobs_df.empty:
        ...
        st.dataframe(format_dates_for_display(jobs_df[["job_code", "job_name", "region_code", "customer", "location", "job_start_date", "job_end_date", "mob_start_date", "demob_end_date", "status"]]), use_container_width=True)
...
with tab_requirements:
    ...
    if req_summary.empty:
        ...
        st.dataframe(format_dates_for_display(req_summary[[
...
with tab_allocations:
    ...
    if req_summary.empty:
        ...
        st.dataframe(format_dates_for_display(req_summary[[
            "job_code", "job_name", "region_code", "class_name", "quantity_required", "quantity_assigned", "quantity_shortfall", "allocation_status"
        ]]), use_container_width=True)
    st.subheader("Fulfillment Rows")
    if ful.empty:
        st.info("No fulfillment rows yet.")
    else:
        st.dataframe(format_dates_for_display(ful[["job_code", "job_name", "region_code", "class_name", "fulfillment_type", "source_name", "specific_resource_name", "quantity_assigned", "required_start", "required_end"]]), use_container_width=True)
...
with tab_calendar:
    ...
    if mode == "Availability":
        ...
        if snapshot.empty:
            ...
            st.dataframe(format_dates_for_display(snapshot[["region_code", "class_name", "unit_type", "total_pool", "committed_quantity", "available_quantity"]]), use_container_width=True)
    elif mode == "Demand":
        req_summary = requirement_summary_df(engine)
        if req_summary.empty:

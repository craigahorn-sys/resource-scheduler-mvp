...
with tab_jobs:
    ...
    with st.form("create_job_form", clear_on_submit=True):
        ...
        st.info(f"Job End: {dates_preview['job_end_date']}  |  Mob Start: {dates_preview['mob_start_date']}  |  Demob End: {dates_preview['demob_end_date']}")
...
with tab_calendar:
    ...
    if mode == "Availability":
        ...
        if snapshot.empty:
            ...
            st.dataframe(snapshot[["region_code", "class_name", "unit_type", "total_pool", "committed_quantity", "available_quantity"]], use_container_width=True)
    elif mode == "Demand":
        req_summary = requirement_summary_df(engine)
        if req_summary.empty:

...
def calc_job_dates(job_start_date, job_duration_days: int, mob_days_before_job: int, demob_days_after_job: int):
    ...
    job_end = start + timedelta(days=max(int(job_duration_days), 1) - 1)
    mob_start = start - timedelta(days=max(int(mob_days_before_job), 0))
    demob_end = job_end + timedelta(days=max(int(demob_days_after_job), 0))
    return {

...
def create_job(engine, data: dict) -> int:
    ...
    with engine.begin() as conn:
        ...
        try:
            ...
            return conn.execute(text("SELECT id FROM jobs WHERE job_code=:job_code"), {'job_code': job_code}).scalar()
...
def upsert_pool(engine, region_code: str, resource_class_id: int, base_quantity: float, notes: str = ''):
    existing = _scalar(engine, "SELECT id FROM resource_pools WHERE region_code=:region_code AND resource_class_id=:resource_class_id", {'region_code': region_code, 'resource_class_id': resource_class_id})
    if existing:
        execute(engine, "UPDATE resource_pools SET base_quantity=:base_quantity, notes=:notes WHERE id=:id", {'base_quantity': base_quantity, 'notes': notes, 'id': existing})
    else:
        execute(engine, "INSERT INTO resource_pools(region_code, resource_class_id, base_quantity, notes) VALUES (:region_code,:resource_class_id,:base_quantity,:notes)", {'region_code': region_code, 'resource_class_id': resource_class_id, 'base_quantity': base_quantity, 'notes': notes})


def add_pool_adjustment(engine, data: dict):
    execute(engine, "INSERT INTO pool_adjustments(region_code, resource_class_id, quantity_change, adjustment_date, reason, notes) VALUES (:region_code,:resource_class_id,:quantity_change,:adjustment_date,:reason,:notes)", data)
...
def _requirement_window(engine, requirement_id: int):
    df = query_df(engine, """
        ...
        FROM job_requirements jr JOIN jobs j ON jr.job_id=j.id WHERE jr.id=:id
    """, {'id': requirement_id})
    if df.empty:
        return None
    r = df.iloc[0]
    ...
    return {
        'region_code': r['region_code'], 'resource_class_id': int(r['resource_class_id']),
        'required_start': req_start.date(), 'required_end': req_end.date(), 'quantity_required': float(r['quantity_required'])
    }
...
def _recalc_requirement_allocation(engine, requirement_id: int):
    ...
    existing = _scalar(engine, "SELECT id FROM requirement_fulfillment WHERE requirement_id=:rid AND fulfillment_type='internal_pool'", {'rid': requirement_id})
    if existing and assign_qty > 0:
        execute(engine, "UPDATE requirement_fulfillment SET quantity_assigned=:q, source_name='Internal Pool' WHERE id=:id", {'q': assign_qty, 'id': existing})
    elif existing and assign_qty <= 0:
        execute(engine, "DELETE FROM requirement_fulfillment WHERE id=:id", {'id': existing})
    elif assign_qty > 0:
        execute(engine, "INSERT INTO requirement_fulfillment(requirement_id, fulfillment_type, source_name, specific_resource_name, quantity_assigned, notes) VALUES (:rid,'internal_pool','Internal Pool',NULL,:q,'Auto-allocated')", {'rid': requirement_id, 'q': assign_qty})
...
def create_requirement(engine, data: dict):
    with engine.begin() as conn:
        ...
        """), data)
        try:
            requirement_id = res.inserted_primary_key[0]
        except Exception:
            requirement_id = conn.execute(text("SELECT max(id) FROM job_requirements")).scalar()
...
def get_fulfillment_df(engine):
    df = query_df(engine, """
        SELECT rf.*, j.job_code, j.job_name, j.region_code, rc.class_name, rc.unit_type,
               date(j.job_start_date, '-' || jr.days_before_job_start || ' day') AS required_start,
               date(j.job_start_date, '+' || (j.job_duration_days - 1 + jr.days_after_job_end) || ' day') AS required_end
        FROM requirement_fulfillment rf
        JOIN job_requirements jr ON rf.requirement_id=jr.id
        JOIN jobs j ON jr.job_id=j.id
        JOIN resource_classes rc ON jr.resource_class_id=rc.id
...
def pool_snapshot_df(engine, as_of_date):
    df = query_df(engine, """
        ...
        FROM resource_pools rp JOIN resource_classes rc ON rp.resource_class_id=rc.id
    ...
    for _, r in df.iterrows():
        q = 0.0
        if not req.empty:
            mask = (req['region_code']==r['region_code']) & (req['class_name']==r['class_name']) & (pd.to_datetime(req['required_start']) <= pd.to_datetime(as_of_date)) & (pd.to_datetime(req['required_end']) >= pd.to_datetime(as_of_date))
            q = float(req.loc[mask, 'quantity_assigned'].sum())
        committed.append(q)
    df['total_pool'] = df['base_quantity'].astype(float) + df['adjustment_total'].astype(float)

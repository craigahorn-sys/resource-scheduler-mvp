...
def update_job(engine, job_id: int, data: dict):
    ...
    execute(engine, """
        UPDATE jobs
        SET job_name=:job_name, region_code=:region_code, customer=:customer, location=:location,
...
def requirement_summary_df(engine):
    df = query_df(engine, """
        SELECT
            jr.id, j.job_code, j.job_name, j.region_code, rc.class_name, rc.unit_type,
        ...
        GROUP BY jr.id, j.job_code, j.job_name, j.region_code, rc.class_name, rc.unit_type,
            jr.quantity_required, jr.days_before_job_start, jr.days_after_job_end, jr.priority, jr.notes,
            j.job_start_date, j.job_duration_days, j.mob_days_before_job, j.demob_days_after_job
        ORDER BY jr.id DESC
    """)
...
def pool_snapshot_df(engine, as_of_date):
    df = query_df(engine, """
        SELECT
            rp.id, rp.region_code, rc.class_name, rc.category, rc.unit_type, rp.base_quantity,
    ...
    return df[["id","region_code","class_name","category","unit_type","base_quantity","adjustment_total","total_pool","committed_quantity","available_quantity","pool_status"]]

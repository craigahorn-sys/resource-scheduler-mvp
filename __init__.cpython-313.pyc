...
def rebalance_region_class(engine, region_code: str, resource_class_id: int):
    ...
    global_start = min(pd.to_datetime(req["required_start"]).dt.date)
    global_end = max(pd.to_datetime(req["required_end"]).dt.date)
    capacity = _capacity_by_day(engine, region_code, resource_class_id, global_start, global_end)
    remaining = {day: float(qty) for day, qty in capacity.items()}
    ...
    for _, row in ordered.iterrows():
        ...
        allocations[int(row["id"])] = {
            "assigned": float(alloc),
            "min_remaining_before_assignment": float(min_remaining),
        }

    req_ids = [int(v) for v in ordered["id"].tolist()]
    if req_ids:
        placeholders = ", ".join([f":rid_{i}" for i in range(len(req_ids))])
        params = {f"rid_{i}": rid for i, rid in enumerate(req_ids)}
        execute(
            engine,
            f"DELETE FROM requirement_fulfillment WHERE fulfillment_type = 'internal_pool' AND requirement_id IN ({placeholders})",
            params,
        )

    for rid, alloc_info in allocations.items():
        assigned = float(alloc_info["assigned"])
        if assigned <= 0:
            continue
        execute(
            engine,
...
def requirement_summary_df(engine):
    ...
    for (region_code, resource_class_id), grp in req.groupby(["region_code", "resource_class_id"]):
        grp = grp.copy()
        global_start = min(pd.to_datetime(grp["required_start"]).dt.date)
        global_end = max(pd.to_datetime(grp["required_end"]).dt.date)
...
def pool_snapshot_df(engine, as_of_date):
    ...
    for _, pool in pools.iterrows():
        related = req.loc[
            ...
            & (pd.to_datetime(req["required_start"]).dt.date <= as_of_date)
            & (pd.to_datetime(req["required_end"]).dt.date >= as_of_date)
        ].copy()
